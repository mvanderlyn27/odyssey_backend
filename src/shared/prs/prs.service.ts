...
export class PrService {
  ...
  async calculateUserExercisePRs(
    ...
    const module = "prs";
    this.log.info(
      { userId: user.id, setsCount: persistedSessionSets.length, module },
      "[PrService] Starting PR calculation"
    );
    ...
    try {
      ...
      for (const [exerciseId, sets] of setsByExercise.entries()) {
        ...
        this.log.debug(
          { userId: user.id, exerciseId, setCount: sets.length, existingPRs, module },
        ...
        if (!isBodyweightExercise && sessionBest1RMSet) {
          ...
          if (!existing1RM) {
            ...
            this.log.debug({ userId: user.id, payload, module }, "[PrService] New 1RM PR (no existing) payload");
            ...
            const payload = createPrPayload(sessionBest1RMSet, "one_rep_max");
            this.log.debug(
              { userId: user.id, payload, existing: existing1RM, module },
              "[PrService] New 1RM PR (beat existing) payload"
            );
            newPrsToUpsert.push(payload);
            newPrsForFeed.push({ ...payload, exercise_name: exerciseInfo.name });
          }
        ...
        if (sessionBestRepsSet) {
          ...
          if (!existingMaxReps) {
            ...
            this.log.debug({ userId: user.id, payload, module }, "[PrService] New Max Reps PR (no existing) payload");
            newPrsToUpsert.push(payload);
            newPrsForFeed.push({ ...payload, exercise_name: exerciseInfo.name });
          } else if ((sessionBestRepsSet.actual_reps ?? -1) > (existingMaxReps.reps ?? -1)) {
            const payload = createPrPayload(sessionBestRepsSet, "max_reps");
            this.log.debug(
              { userId: user.id, payload, existing: existingMaxReps, module },
              "[PrService] New Max Reps PR (beat existing) payload"
            );
            newPrsToUpsert.push(payload);
            newPrsForFeed.push({ ...payload, exercise_name: exerciseInfo.name });
          }
        ...
        if (!isBodyweightExercise && sessionBestSWRSet) {
          ...
          if (!existingMaxSWR) {
            ...
            this.log.debug({ userId: user.id, payload, module }, "[PrService] New Max SWR PR (no existing) payload");
            newPrsToUpsert.push(payload);
            newPrsForFeed.push({ ...payload, exercise_name: exerciseInfo.name });
          } else if ((sessionBestSWRSet.calculated_swr ?? -1) > (existingMaxSWR.swr ?? -1)) {
            const payload = createPrPayload(sessionBestSWRSet, "max_swr");
            this.log.debug(
              { userId: user.id, payload, existing: existingMaxSWR, module },
              "[PrService] New Max SWR PR (beat existing) payload"
            );
            newPrsToUpsert.push(payload);
            newPrsForFeed.push({ ...payload, exercise_name: exerciseInfo.name });
          }
      ...
      if (newPrsToUpsert.length > 0) {
        const module = "prs";
        this.log.info({ userId: user.id, count: newPrsToUpsert.length, module }, "[PrService] Upserting new PRs");
        const { error } = await this.supabase
          .from("user_exercise_prs")
          .upsert(newPrsToUpsert, { onConflict: "user_id,exercise_key,pr_type" });
        if (error) {
          throw new Error(`Error upserting new PRs: ${error.message}`);
        }
      }
      ...
      const module = "prs";
      this.log.error(
        { error, userId: user.id, persistedSessionSets, module },
        "[PrService] Error calculating user exercise PRs"
      );
      // We don't rethrow here because PR calculation is not a critical path.
      // The workout session should still be saved.
      return [];
    }
  }
}
