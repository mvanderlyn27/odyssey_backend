...
export class RankingService {
  ...
  public async updateUserRanks(
    ...
    this.log.info({ userId, source, module }, "[RankingService] Starting updateUserRanks");
    try {
      if (!userBodyweight) {
        this.log.warn({ userId, source, module }, "[RankingService] Returning early due to missing userBodyweight.");
      ...
      for (const [muscleGroupId, newScore] of muscleGroupScores.entries()) {
        ...
        if (!initialRank || newScore > (initialRank?.leaderboard_score ?? 0)) {
          ...
          this.log.debug(
            { userId, source, module, muscleGroupId, newScore, newPermanentScore },
      ...
      for (const [muscleId, newScore] of muscleScores.entries()) {
        ...
        if (!initialRank || newScore > (initialRank?.leaderboard_score ?? 0)) {
          ...
          this.log.debug(
            { userId, source, module, muscleId, newScore, newPermanentScore },
            "[RankingService] Processing muscle"
          );
      ...
      for (const [exerciseId, { score: newScore, set: bestSet }] of exerciseScores.entries()) {
        ...
        this.log.debug(
          { userId, source, module, exerciseId, newScore, newPermanentScore },
          "[RankingService] Processing exercise"
        );
      ...
      this.log.info({ userId, source, module }, "[RankingService] Finished updateUserRanks");
      this.log.debug(
        {
          userId,
          source,
      ...
      this.log.trace(
        ...
        "[RankingService] Final ranking payload"
      );
      return { rankUpData, rankUpdatePayload };
    } catch (error) {
      const module = "ranking";
      this.log.error({ error, userId, source, module }, "[RankingService] Error updating user ranks");
      throw error;
    }
  }
