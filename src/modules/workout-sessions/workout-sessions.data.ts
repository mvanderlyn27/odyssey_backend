...
export async function _gatherAndPrepareWorkoutData(
  ...
  try {
    ...
    const [
      ...
      (async () => {
        ...
        if (customExercisesRes.error) {
          throw new Error(`Error fetching custom exercises: ${customExercisesRes.error.message}`);
        }
    ...
    if (bodyWeightResult.error) {
      fastify.log.debug(
        { error: bodyWeightResult.error, userId, module },
    ...
    if (userBodyweight === null) {
      fastify.log.debug(
        { userId, performedAt: finishData.ended_at, module },
    ...
    if (exercisesDataResult.error) {
      fastify.log.debug(
        { error: exercisesDataResult.error, userId, module },
        "[PREPARE_WORKOUT_DATA] Error fetching exercise details."
      );
    ...
    if (emgMappingsResult.error) {
      fastify.log.debug(
        { error: emgMappingsResult.error, userId, module },
    ...
    if (customEmgMappingsResult.error) {
      fastify.log.debug(
        { error: customEmgMappingsResult.error, userId, module },
    ...
    if (existingPRsResult.error) {
      fastify.log.debug(
        { error: existingPRsResult.error, userId, module },
        "[PREPARE_WORKOUT_DATA] Error fetching existing PRs."
      );
    ...
    if (userExerciseRanksResult.error) {
      fastify.log.debug(
        { error: userExerciseRanksResult.error, userId, module },
    ...
    const module = "workout-sessions";
    fastify.log.error({ userId, error, module }, "[PREPARE_WORKOUT_DATA] Error preparing workout data");
    fastify.posthog?.capture({
      distinctId: userId,
      event: "prepare_workout_data_error",
      properties: {
